...
from ganpaint import pbar
from ganpaint import zdataset, seededsampler, zipdataset
from ganpaint import proggan, subsequence, customnet, parallelfolder
from ganpaint import encoder_net, encoder_loss, imagedata
from ganpaint.imagedata import load_images
from torchvision import transforms, models
from torchvision.models.vgg import model_urls
from ganpaint.pidfile import exit_if_job_done, mark_job_done
from ganpaint import nethook
from ganpaint.pidfile import exit_if_job_done, mark_job_done
from ganpaint.encoder_loss import cor_square_error
from ganpaint.nethook import InstrumentedModel
...
parser.add_argument('--check_edit', type=int,
        help='Set to 1 to generate ablation tests',
        default=0)
parser.add_argument('--second_phase', type=int,
        help='Set to 1 to do second phase search for z',
        default=0)
...
if args.check_edit:
    EDIT_NAME, EDIT_UNITS = imagedata.neurons(model=args.model)[0]
...
def main():
    ...
    unwrapped_H = gan_generator.subsequence(last_layer='layer4')
    # Edit the output of this layer
    F = gan_generator.subsequence(first_layer='layer5')

    # Load a pretrained gan inverter
    encoder = nethook.InstrumentedSequence(
    ...
    if args.check_edit and (true_r is not None):
        with torch.no_grad():
            visualize_results(
                    (image_number, 'target', EDIT_NAME),
                    F(edit(true_r)),
                    summarize=True)
    ...
    for step_num in pbar(range(num_steps + 1)):
        ...
        if (step_num % args.snapshot_every == 0) or (step_num == num_steps):
            with torch.no_grad():
                ...
                if args.check_edit:
                    visualize_results(
                            (image_number, 'a', step_num, EDIT_NAME),
                            F(edit(current_r)),
                            summarize=(step_num in [0, num_steps]))
                for s in residuals:
                    s = s.replace('layer', '')
    ...
    # For the second stage, just optimize over z.
    H = encoder_net.ResidualGenerator(unwrapped_H, init_z, ['z'])
    H.eval()
    H.cuda()

    # Set up optimizer for phase 2
    set_requires_grad(False, F, H, E, D, VF)
    parameters = OrderedDict(H.named_parameters(recurse=False))
    for n, p in parameters.items():
        p.requires_grad = True
    ...
    # Phase 2: match the target r4 by finding a good z.
    for step_num in pbar(range(num_steps + 1)):
        current_r = H()
        current_p = F(current_r)
        current_v = VF(current_p)
        ...
        loss_v = l1_loss(true_v, current_v)
        loss_z = H.dz.pow(2).mean() if hasattr(H, 'dz') else 0
        loss_r = mse_loss(target_r, current_r)
        loss = (loss_p + loss_v + loss_z + loss_r)
        ...
        if (step_num % args.snapshot_every == 0) or (step_num == num_steps):
            with torch.no_grad():
                current_z = init_z + H.dz
                with InstrumentedModel(unwrapped_H) as inst_H:
                    inst_H.retain_layers(['layer1', 'layer2', 'layer3'])
                    inst_H(current_z)
                    current_r1, current_r2, current_r3 = [
                            inst_H.retained_layer(n)
                            for n in ['layer1', 'layer2', 'layer3']]
                if true_z is not None:
                    all_loss['err_z'] = cor_square_error(current_z, true_z
                            ) * 100
                if true_r is not None:
                    all_loss['err_r'] = cor_square_error(current_r, true_r
                            ) * 100
                if true_r1 is not None:
                    all_loss['err_r1'] = cor_square_error(current_r1, true_r1
                            ) * 100
                if true_r2 is not None:
                    all_loss['err_r2'] = cor_square_error(current_r2, true_r2
                            ) * 100
                if true_r3 is not None:
                    all_loss['err_r3'] = cor_square_error(current_r3, true_r3
                            ) * 100
                all_loss['err_p'] = (current_p - true_p).pow(2).mean() * 100
                log_progress('%d ' % step_num + ' '.join(
                    '%s=%.3f' % (k, all_loss[k])
                    for k in sorted(all_loss.keys())), phase='b')
                visualize_results((image_number, 'b', step_num), current_p,
                        summarize=(step_num in [0, num_steps]))
                checkpoint_dict = OrderedDict(all_loss)
                if args.check_edit:
                    visualize_results(
                            (image_number, 'b', step_num, EDIT_NAME),
                            F(edit(current_r)),
                            summarize=(step_num in [0, num_steps]))
                for s in ['z']:
                    s = s.replace('layer', '')
